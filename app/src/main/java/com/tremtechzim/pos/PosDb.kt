...
class PosDb(c:Context):SQLiteOpenHelper(c,"tremtech_pos.db",null,2){
 override fun onCreate(db:SQLiteDatabase){
  ...
  db.execSQL("CREATE TABLE sale_items(id INTEGER PRIMARY KEY AUTOINCREMENT,sale_id INTEGER,product_id INTEGER,stock_unit_id INTEGER,quantity REAL,unit_price REAL,description TEXT)")
 ...
 fun completeSale(items:List<CartItem>,payment:String,customerId:Long?):Long{
  ...
  try{
   ...
   items.forEach{it->
    d.execSQL("INSERT INTO sale_items(sale_id,product_id,stock_unit_id,quantity,unit_price,description) VALUES(?,?,?,?,?,?)",arrayOf(sale,it.productId,if(it.kind=="UNIT")it.id else null,it.qty,it.price,it.name))
    if(it.kind=="UNIT")d.execSQL("UPDATE stock_units SET status='SOLD' WHERE id=? AND status='AVAILABLE'",arrayOf(it.id))
     addMovement(it.productId,it.id,"SALE",it.qty,"Sale #$sale",null)
    else{d.execSQL("UPDATE stock_batches SET quantity=quantity-? WHERE id=? AND quantity>=?",arrayOf(it.qty,it.id,it.qty));d.execSQL("UPDATE stock_batches SET status='OUT_OF_STOCK' WHERE id=? AND quantity<=0",arrayOf(it.id))
 ...
 fun completeSaleWithTerms(items:List<CartItem>, payment:String, customerId:Long?, paid:Double, dueDate:String, reference:String):Long {
  ...
  try {
   val total=items.sumOf{it.price*it.qty}
   val status=if(payment=="Credit") "CREDIT" else if(payment=="Layby") "LAYBY" else "PAID"
   val s=d.compileStatement("INSERT INTO sales(customer_id,total,payment,status,created_at) VALUES(?,?,?,?,?)")
   ...
   items.forEach{it->
    d.execSQL("INSERT INTO sale_items(sale_id,product_id,stock_unit_id,quantity,unit_price,description) VALUES(?,?,?,?,?,?)",arrayOf(sale,it.productId,if(it.kind=="UNIT")it.id else null,it.qty,it.price,it.name))
    if(payment=="Layby" && it.kind=="UNIT") d.execSQL("UPDATE stock_units SET status='RESERVED' WHERE id=? AND status='AVAILABLE'",arrayOf(it.id))
    else if(payment=="Credit" && it.kind=="UNIT") d.execSQL("UPDATE stock_units SET status='SOLD' WHERE id=? AND status='AVAILABLE'",arrayOf(it.id))
     addMovement(it.productId,it.id,"SALE",it.qty,"Sale #$sale",null)
    else if(payment!="Layby" && it.kind=="UNIT") d.execSQL("UPDATE stock_units SET status='SOLD' WHERE id=? AND status='AVAILABLE'",arrayOf(it.id))
     addMovement(it.productId,it.id,"SALE",it.qty,"Sale #$sale",null)
    else if(payment!="Layby"){d.execSQL("UPDATE stock_batches SET quantity=quantity-? WHERE id=? AND quantity>=?",arrayOf(it.qty,it.id,it.qty));d.execSQL("UPDATE stock_batches SET status='OUT_OF_STOCK' WHERE id=? AND quantity<=0",arrayOf(it.id))
     addMovement(it.productId,null,"SALE",it.qty,"Sale #$sale",null)}
   }
   ...
   if(payment=="Credit") d.execSQL("INSERT INTO credit_accounts(sale_id,customer_id,original_amount,balance,due_date,status) VALUES(?,?,?,?,?,?)",arrayOf(sale,customerId,total,balance,dueDate,if(balance<=0)"PAID" else "OPEN"))
   if(payment=="Layby"){
    val lay=d.compileStatement("INSERT INTO laybys(sale_id,customer_id,total,balance,status,release_status) VALUES(?,?,?,?,?,?)")
    lay.bindLong(1,sale);if(customerId==null)lay.bindNull(2)else lay.bindLong(2,customerId);lay.bindDouble(3,total);lay.bindDouble(4,balance);lay.bindString(5,if(balance<=0)"PAID" else "OPEN");lay.bindString(6,if(balance<=0)"READY_TO_RELEASE" else "HELD");val lid=lay.executeInsert()
 ...
 fun profitRows(limit:Int=100):List<ProfitRow>{
  ...
  readableDatabase.rawQuery("""SELECT s.id,s.created_at,s.total,
   COALESCE((SELECT SUM(si.quantity*COALESCE(su.cost_price,sb.cost_price)) FROM sale_items si
    ...
    LEFT JOIN stock_batches sb ON sb.id=si.stock_unit_id WHERE si.sale_id=s.id),0)
 ...
 fun exportRows(table:String):List<Array<String>>{
  val safe=when(table){ "products"->"SELECT id,name,brand,model,sku,category FROM products";"customers"->"SELECT id,name,phone,whatsapp,email,address FROM customers";"sales"->"SELECT id,customer_id,total,payment,status,created_at FROM sales";"movements"->"SELECT id,product_id,stock_unit_id,movement_type,quantity,reference,created_at FROM stock_movements";else->return emptyList()}
  val r=mutableListOf<Array<String>>();readableDatabase.rawQuery(safe,null).use{c->val n=c.columnCount;val h=Array(n){i->c.getColumnName(i)};r+=h;while(c.moveToNext())r+=Array(n){i->c.getString(i)?:""}};return r
 }
 ...
 fun refundSale(saleId:Long,amount:Double,reason:String,userId:Long?){
  val d=writableDatabase;d.beginTransaction();try{
   ...
   audit(userId,"SALE_REFUND","Sale #$saleId $reason")
 ...
 fun dashboard():Dashboard{
  ...
  return Dashboard(sales,expenses,0.0,costUnits+costBatch,receivables,layby,tx)
 }
 ...
 fun lowStock(limit:Double=3.0):List<Pair<String,Double>>{
  val r=mutableListOf<Pair<String,Double>>();readableDatabase.rawQuery("SELECT p.name,b.quantity FROM stock_batches b JOIN products p ON p.id=b.product_id WHERE b.status='AVAILABLE' AND b.quantity<=? ORDER BY b.quantity",arrayOf(limit.toString())).use{c->while(c.moveToNext())r+=c.getString(0) to c.getDouble(1)}};return r
 }
 ...
 fun totals():Totals{fun s(t:String):Double=readableDatabase.rawQuery("SELECT COALESCE(SUM("+if(t=="sales")"total" else "amount"+"),0) FROM $t WHERE created_at LIKE ?",arrayOf("${today()}%")).use{it.moveToFirst();it.getDouble(0)};val a=s("sales");val e=s("expenses");return Totals("%.2f".format(a),"%.2f".format(e),"%.2f".format(a-e))}
}
